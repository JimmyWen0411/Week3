...
const fs = require('fs');
const url = "https://en.wikipedia.org/wiki/Capitalism"; // FIRST: find a url of a page you are interested in from wikipedia 
const jsonPath = "./json/"; 
...
function getParagraphText(soupTag){
    ...
    for(let i = 0; i < paragraphs.length; i++){
        ...
        if(p.indexOf("economic") != -1){ //if text has plant
        console.log(p);
        text += p;
        sentence = text.split('. ')
    }
